...
return function (Router $router, array $config) {
    ...
    $router->post('/sso/create-link',   fn(Request $r) => $sso->createLink($r));
    $router->post('/sso/exchange',      fn(Request $r) => $sso->exchange($r));
    $router->get('/sso/code-status',    fn(Request $r) => $sso->codeStatus($r));
    ...
    // Mentions — @-suggest + saved Mentions Smart View backend
    $mentions = new MentionsController($config);
    $router->get('/mentions/messages',    fn(Request $r) => $mentions->listMessages($r));
    ...
    $router->post('/mentions/backfill',   fn(Request $r) => $mentions->backfill($r));
    ...
    if ($mayBoardProRoute) {
        ...
        if ($addonSvc->isKanbanBoardsEnabled() && $addonSvc->isBoardProEnabled()) {
            ...
            $router->put('/board-pro/cards/{card_id}/permissions', fn(Request $r) => $boardPro->updateCardPermissions($r));
            $router->get('/board-pro/cards/{card_id}/permissions', fn(Request $r) => $boardPro->getCardPermissions($r));
            $router->put('/board-pro/boards/{id}/stage-permissions', fn(Request $r) => $boardPro->updateStagePermissions($r));
            $router->get('/board-pro/boards/{id}/stage-permissions', fn(Request $r) => $boardPro->getStagePermissions($r));
    ...
    $router->post('/auth/google/callback', fn(Request $r) => $accounts->googleCallbackPost($r));
    ...
    $router->post('/auth/microsoft/callback', fn(Request $r) => $accounts->microsoftCallbackPost($r));
    ...
    $router->get('/drive/files/{id}/versions', fn(Request $r) => $drive->getVersions($r));
    $router->post('/drive/files/{id}/versions/{versionId}/restore', fn(Request $r) => $drive->restoreVersion($r));
    $router->delete('/drive/files/{id}/versions/{versionId}', fn(Request $r) => $drive->deleteVersion($r));
    $router->get('/drive/files/{id}/versions/{versionId}/download', fn(Request $r) => $drive->downloadVersion($r));
    $router->get('/drive/files/{id}/versions/{versionId}/preview', fn(Request $r) => $drive->previewVersion($r));
    ...
    $router->get('/calendar/connections/callback', fn(Request $r) => $calendarConn->callback($r));
    $router->post('/calendar/connections/callback', fn(Request $r) => $calendarConn->callbackPost($r));
    ...
    // Push Notification routes (Web Push for PWA)
