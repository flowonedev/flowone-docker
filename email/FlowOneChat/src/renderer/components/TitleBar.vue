...
onMounted(async () => {
  checkMaximized()
  if (window.api) window.addEventListener('resize', checkMaximized)
  if (window.api?.network?.onStatusChange) {
    window.api.network.onStatusChange((online) => { isOffline.value = !online })
  }
...
<template>
  <div class="h-8 flex items-center select-none draggable-region border-b bg-white dark:bg-surface-900 border-surface-200 dark:border-surface-800">
    ...
    <div v-if="!isMac" class="flex items-center gap-2 px-3 h-full non-draggable shrink-0">
      ...
      <span class="text-xs font-medium text-surface-600 dark:text-surface-300">FlowOne - Chat</span>
    ...
    <div v-if="!isMac" class="flex items-center h-full non-draggable">
      <button @click="minimize" class="h-full w-11 flex items-center justify-center hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors" title="Minimize">
        <span class="material-symbols-rounded text-base text-surface-500 dark:text-surface-400">remove</span>
      </button>
      <button @click="maximize" class="h-full w-11 flex items-center justify-center hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors" :title="isMaximized ? 'Restore' : 'Maximize'">
        <span class="material-symbols-rounded text-base text-surface-500 dark:text-surface-400">{{ isMaximized ? 'filter_none' : 'crop_square' }}</span>
      </button>
      <button @click="close" class="close-btn h-full w-11 flex items-center justify-center hover:bg-red-600 transition-colors" title="Close">
...
.close-btn:hover .material-symbols-rounded { color: white; }
</style>
