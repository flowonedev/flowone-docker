 ...
 *   Orphan messages (IMAP returns null) are left alone — the mailsync
 *   worker's QRESYNC expunge events propagate external deletions and
 *   the next CONDSTORE delta reconciles surviving rows.
 *
...
foreach ($users as $userEmail) {
    ...
    foreach ($candidates as $msg) {
        ...
        try {
            ...
            // Leave has_attachment alone — orphans are removed by the mailsync
            // worker (idleManager QRESYNC expunge events); we shouldn't silently
            // clear flags here from a transient miss.
            if ($full === null) {
                $orphans++;
                $totalOrphans++;
...
function resolveCredentials(PDO $db, string $userEmail, ?string $explicitPwd, array $config): ?array
{
    ...
    // 1. OAuth accounts (Gmail / Outlook etc.) — Phase 2.5 fix.
    //
    // Previously this queried a non-existent `email_accounts` table with
    // plaintext access_token / refresh_token columns, so no OAuth user ever
    // matched and the cron silently fell through to the session-password
    // path. Tokens really live in `webmail_oauth_tokens` AES-256-GCM
    // encrypted; the canonical way to obtain a usable access token is via
    // GoogleOAuthService::getValidAccessToken() (which handles lazy refresh
    // and health tracking). Mirrors process-scheduled-emails.php.
    try {
        $stmt = $db->prepare("
        ...
        if ($row) {
            ...
            try {
                ...
                error_log("register-attachments resolveCredentials OAuth refresh error: " . $e->getMessage());
            }
            if ($accessToken) {
                return [
        ...
        // webmail_oauth_tokens may not exist on minimal installs — fall through
        error_log("register-attachments resolveCredentials oauth lookup: " . $e->getMessage());
    }
