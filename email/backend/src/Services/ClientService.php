...
class ClientService
{
    ...
    /** Lazily build the address-book service for canonical-person linking. */
    private function addressBook(): ?\Webmail\Addons\Contacts\Services\AddressBookService
    {
        if ($this->addressBook === null) {
            try {
                ...
                error_log('ClientService addressBook init error: ' . $e->getMessage());
                return null;
            }
        }
    ...
    /**
     * Link a client_contacts row to the canonical address-book person (creating
     * that person in the non-synced "Other contacts" pool if needed). Best-effort:
     * never throws into the client write path. The CRM keeps using
     * client_contacts.id; this only adds the shared identity.
     */
    private function linkClientContactToCanonical(int $clientId, string $email, ?string $name): void
    {
        try {
            $owner = $this->db->prepare('SELECT user_email FROM clients WHERE id = ?');
            $owner->execute([$clientId]);
            $userEmail = (string) ($owner->fetchColumn() ?: '');
            if ($userEmail === '') {
                return;
            }
            $ab = $this->addressBook();
            ...
            $contact = $ab->findOrCreateByEmail($userEmail, $email, $name, 'client');
            if ($contact) {
                $this->db->prepare('UPDATE client_contacts SET contact_id = ? WHERE client_id = ? AND email = ? AND (contact_id IS NULL OR contact_id <> ?)')
                    ->execute([(int) $contact['id'], $clientId, strtolower(trim($email)), (int) $contact['id']]);
            }
        } catch (\Throwable $e) {
            error_log('ClientService linkClientContactToCanonical error: ' . $e->getMessage());
        }
    }
    ...
    private function ensureTablesExist(): void
    {
        ...
        // Migration 185: link to the canonical address-book person (additive only;
        // client_contacts.id stays the CRM-facing key, nothing re-points).
        $this->addColumnIfNotExists('client_contacts', 'contact_id', 'INT NULL AFTER email_count');
    ...
    public function addContact(int $clientId, string $email, ?string $name = null, ?string $phone = null, ?string $position = null): ?array
    {
        ...
        try {
            ...
            if ($existing) {
                ...
                $this->linkClientContactToCanonical($clientId, $email, $name);
