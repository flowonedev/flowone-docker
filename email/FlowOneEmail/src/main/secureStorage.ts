/**
 * Secure Storage Module for FlowOneEmail
 * 
 ...
 * 
 * When safeStorage is unavailable (e.g. Linux without a keyring), falls back to
 * AES-256-GCM software encryption using a machine-derived key via PBKDF2.
 * 
 * Non-sensitive config (UI preferences, window state) stays in electron-store (plaintext).
 * Sensitive data (auth tokens, session tokens) goes through safeStorage.
 */
...
/**
 * Get the OS machine identifier for key derivation
 */
...
export function getSecure(key: keyof SecureData): string | null {
  ...
  try {
    ...
    // If decryption fails (e.g., after OS reinstall), clear the value
    encryptedStore.delete(key)
    return null
  }
}
