...
onMounted(async () => {
  const cacheKey = `mood_board:${props.id}`
  ...
  try {
    const res = await api.get('/chat/embed/resolve', { params: { type: 'mood_board', embed_id: props.id } })
    if (res.data.success) {
...
function handleClick() {
  window.dispatchEvent(new CustomEvent('navigate-to', { detail: { view: 'mood', boardId: props.id } }))
}
</script>

<template>
  <div 
    class="embed-card group cursor-pointer rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800 overflow-hidden hover:border-pink-300 dark:hover:border-pink-600 transition-colors w-64"
    @click="handleClick"
  >
    <!-- Loading state -->
    ...
    <div v-else-if="error || !data" class="flex items-center gap-3 p-3 text-surface-400">
      ...
      <span class="text-xs">Mood board not available</span>
    </div>
    ...
    <div v-else>
      ...
      <div class="flex items-center gap-3 p-3">
        <div class="w-10 h-10 rounded-lg bg-pink-50 dark:bg-pink-900/30 flex items-center justify-center flex-shrink-0">
          <span class="material-symbols-rounded text-pink-500 dark:text-pink-400 text-xl">palette</span>
        </div>
        ...
        <span class="material-symbols-rounded text-surface-300 group-hover:text-pink-500 text-lg transition-colors flex-shrink-0">open_in_new</span>
      </div>
    </div>
  </div>
</template>
