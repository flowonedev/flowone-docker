...
const hasTriggerConfig = computed(() => needsTriggerStage.value || needsTriggerDays.value || needsTriggerThreshold.value || needsTriggerTaskConfig.value || needsTriggerTimeSpent.value || needsTriggerSickKeyword.value || needsTriggerDriveFolder.value)
  ...
  <div class="flex-1 overflow-auto">
    ...
    <div class="sticky top-0 z-10 flex items-center justify-between px-6 py-4 border-b border-surface-200 dark:border-[rgb(var(--color-border))] bg-white dark:bg-[rgb(var(--color-surface))]">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-surface-400">Define a trigger condition and the action to perform</p>
        </div>
      </div>
      <div class="flex items-center gap-3">
    ...
    <div class="p-6 space-y-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div>
          <label :class="labelClass">Rule Name *</label>
          <input v-model="form.name" placeholder="e.g. Follow up stale proposals" :class="inputClass" />
        </div>
        <div>
          <label :class="labelClass">Description</label>
          <input v-model="form.description" placeholder="What does this rule do?" :class="inputClass" />
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-[rgb(var(--color-surface))] rounded-2xl border border-surface-200 dark:border-[rgb(var(--color-border))] overflow-hidden">
        <div class="px-5 py-4 border-b border-surface-200 dark:border-[rgb(var(--color-border))] flex items-center justify-between">
          <h3 class="text-sm font-semibold text-surface-900 dark:text-white flex items-center gap-2">
            ...
            {{ form.visibility === 'shared' ? 'Shared Rule' : 'Private Rule' }}
          </h3>
          <button
        ...
        <transition name="slide">
          <div v-if="form.visibility === 'shared'" class="p-5 space-y-4">
            <p class="text-xs text-surface-400">Share this automation with colleagues or groups so they can benefit from it. Viewers can see & copy, editors can modify.</p>

            <!-- Group shares -->
            <div>
              <label class="block text-xs font-medium text-surface-600 dark:text-surface-300 mb-1.5">Share with Groups</label>
              ...
              <p v-else-if="!form.shared_groups?.length" class="text-[11px] text-surface-400 italic">No groups available</p>
            </div>

            <!-- Individual shares -->
            <div>
              <label class="block text-xs font-medium text-surface-600 dark:text-surface-300 mb-1.5">Share with Colleagues</label>
              ...
              <div class="relative">
                <div class="flex items-center gap-2 px-3 py-2 rounded-xl border border-surface-300 dark:border-surface-600 bg-white dark:bg-[rgb(var(--color-bg))]">
                  ...
                  <input v-model="sharingSearch" placeholder="Search colleagues to add..." class="w-full text-xs bg-transparent outline-none text-surface-800 dark:text-surface-100 placeholder-surface-400" />
                </div>
          ...
          <div class="p-5 space-y-4">
            ...
            <div v-if="hasTriggerConfig" class="p-4 rounded-xl bg-surface-50 dark:bg-[rgb(var(--color-bg))] space-y-3">
              ...
              <div v-if="needsTriggerStage">
                <label :class="smallLabelClass">Stage</label>
              ...
              <div v-if="needsTriggerDays">
                <label :class="smallLabelClass">Days</label>
              ...
              <div v-if="needsTriggerThreshold">
                <label :class="smallLabelClass">Health Score Threshold</label>
              ...
              <template v-if="needsTriggerTaskConfig">
                <div>
                  <label :class="smallLabelClass">Change Type</label>
                ...
                <div>
                  <label :class="smallLabelClass">Task Status <span class="text-surface-400 font-normal">(optional filter)</span></label>
              ...
              <template v-if="needsTriggerTimeSpent">
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-sm text-surface-500 font-medium whitespace-nowrap">hours</span>
                    </div>
                  </div>
                  <div>
                    <label :class="smallLabelClass">Within period</label>
                ...
                <div>
                  <label :class="smallLabelClass">Track what</label>
                  <select v-model="form.trigger_config.scope" :class="selectClass">
                    <option value="board">Per Project / Board</option>
                    <option value="client">Per Client (all activities)</option>
                  </select>
                </div>
                <!-- Board picker (when scope is board) -->
                <div v-if="form.trigger_config.scope === 'board'">
                  <label :class="smallLabelClass">Specific project <span class="text-surface-400 font-normal">(optional — leave empty for any board)</span></label>
                  <!-- Selected board pill -->
                  ...
                  <div class="rounded-xl border border-surface-200 dark:border-surface-600 bg-white dark:bg-[rgb(var(--color-bg))] overflow-hidden">
                    <div class="px-3 py-2 border-b border-surface-100 dark:border-surface-700">
                      <div class="flex items-center gap-2">
                        ...
                        <input v-model="boardSearchQuery" placeholder="Search boards..." class="w-full text-sm bg-transparent outline-none text-surface-800 dark:text-surface-100 placeholder-surface-400" />
                      </div>
                    </div>
                    <div class="max-h-[180px] overflow-y-auto py-1 px-1">
                      ...
                      <button @click="form.trigger_config.board_id = ''"
                        ...
                        <span class="text-sm">Any board / project</span>
                ...
                <!-- Colleague filter -->
                <div>
                  <label :class="smallLabelClass">Track time by <span class="text-surface-400 font-normal">(who's tracked hours to watch)</span></label>
                  ...
                  <p class="text-[10px] text-surface-400 mt-1">
                    {{ form.trigger_config.colleague_email ? `Tracking hours logged by ${form.trigger_config.colleague_email}` : 'Tracking your own logged hours' }}
                  </p>
                </div>
              </template>
              ...
              <template v-if="needsTriggerSickKeyword">
                <div>
                  <label :class="smallLabelClass">Status keyword to match</label>
                  <input v-model="form.trigger_config.keyword" placeholder="sick" :class="inputClass" />
                  <p class="text-[10px] text-surface-400 mt-1">Triggers when a colleague's status text contains this keyword (case-insensitive)</p>
                </div>
              </template>
              <!-- Drive folder permission config -->
              <template v-if="needsTriggerDriveFolder">
                <div>
                  <label :class="smallLabelClass">Permission Change Type</label>
                  <select v-model="form.trigger_config.change_type" :class="selectClass">
                    <option value="">Any change</option>
                ...
                <div>
                  <label :class="smallLabelClass">Watch Specific Folder <span class="text-surface-400 font-normal">(optional)</span></label>
                  ...
                  <div class="rounded-xl border border-surface-200 dark:border-surface-600 bg-white dark:bg-[rgb(var(--color-bg))] overflow-hidden">
                    ...
                    <div v-if="driveFoldersLoading" class="px-4 py-6 text-center">
                      ...
                      <p class="text-xs text-surface-400 mt-1">Loading folders...</p>
                    </div>
                  ...
                  <p class="text-[10px] text-surface-400 mt-1">Leave empty to watch all your folders</p>
                </div>
              </template>
            </div>
          </div>
            ...
            <div class="p-4 rounded-xl bg-surface-50 dark:bg-[rgb(var(--color-bg))] space-y-3">
              ...
              <template v-if="form.action_type === 'create_reminder'">
                <div>
                  ...
                  <input v-model="form.action_config.title" placeholder="Follow up with {client_name} about {board_name}" :class="inputClass" />
                ...
                <div>
                  <label :class="smallLabelClass">Delay (hours after trigger)</label>
              ...
              <template v-if="form.action_type === 'move_deal_stage'">
                <!-- Deal picker -->
                <div>
                  <label :class="smallLabelClass">Which Deal</label>
                  <p class="text-[11px] text-surface-400 mb-2">Leave empty to move the deal that triggered this rule (for deal-based triggers). Or pick a specific deal:</p>
                  ...
                  <div class="border border-surface-200 dark:border-surface-600 rounded-xl overflow-hidden">
                    <div class="flex items-center gap-2 px-3 py-2 border-b border-surface-200 dark:border-surface-600 bg-surface-50 dark:bg-surface-800/50">
                      ...
                      <input v-model="dealSearchQuery" placeholder="Search deals by name, client, or stage..." class="w-full text-sm bg-transparent outline-none text-surface-800 dark:text-surface-100 placeholder-surface-400" />
                    </div>
                    ...
                    <div v-if="dealsLoading" class="px-4 py-6 text-center">
                      ...
                      <p class="text-xs text-surface-400 mt-1">Loading deals...</p>
                    </div>
                    ...
                    <div v-else class="max-h-56 overflow-y-auto divide-y divide-surface-100 dark:divide-surface-700">
                      ...
                      <button @click="clearDeal"
                        ...
                        <div>
                          <p class="text-sm font-medium" :class="!form.action_config.deal_id ? 'text-primary-700 dark:text-primary-300' : 'text-surface-700 dark:text-surface-200'">Triggered deal (automatic)</p>
                          <p class="text-[11px] text-surface-400">Use whichever deal triggered this rule</p>
                        </div>
                      </button>
                      ...
                      <div v-if="filteredDeals.length === 0 && !dealsLoading" class="px-4 py-4 text-center">
                        <p class="text-xs text-surface-400">No deals found</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <template v-if="form.action_type === 'notify_user'">
                <div>
                  ...
                  <input v-model="form.action_config.message" placeholder="Alert: {board_name} has been closed" :class="inputClass" />
                  <div class="flex flex-wrap gap-1 mt-1.5">
              ...
              <template v-if="form.action_type === 'start_sequence'">
                <div>
                  <label :class="smallLabelClass">Sequence ID</label>
                  <input v-model.number="form.action_config.sequence_id" type="number" min="1" placeholder="Enter sequence ID" :class="inputClass" />
                </div>
              </template>

              <!-- Email config (subject only, body is below) -->
              <template v-if="form.action_type === 'send_email'">
                <div>
                  <label :class="smallLabelClass">Subject</label>
                  <input ref="subjectInput" v-model="form.action_config.subject" placeholder="e.g. Board {board_name} has been closed" :class="inputClass" />
                  <div class="flex flex-wrap gap-1 mt-1.5">
                ...
                <div>
                  <label :class="smallLabelClass">Template ID <span class="text-surface-400 font-normal">(optional, overrides body)</span></label>
                  <input v-model.number="form.action_config.template_id" type="number" min="0" placeholder="Leave empty for custom content" :class="inputClass" />
                </div>
              </template>

              <!-- Invoice draft -->
              <template v-if="form.action_type === 'create_invoice_draft'">
                <p class="text-xs text-surface-400">An invoice draft will be created automatically based on the deal value and client info.</p>
              </template>

              <!-- Assign task config -->
              <template v-if="form.action_type === 'assign_task'">
                <div>
                  <label :class="smallLabelClass">Task Title</label>
                  <input v-model="form.action_config.title" placeholder="e.g. Review {board_name} deliverables" :class="inputClass" />
                  <div class="flex flex-wrap gap-1 mt-1.5">
                    <button v-for="v in templateVars" :key="v.key" @click="insertVarToField('title', v.key)"
                      class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium bg-sky-50 dark:bg-sky-500/10 text-sky-600 dark:text-sky-400 hover:bg-sky-100 dark:hover:bg-sky-500/20 transition-colors cursor-pointer">
                      <span class="material-symbols-rounded" style="font-size:10px">{{ v.icon }}</span>{{ v.key }}
                ...
                <div>
                  <label :class="smallLabelClass">Description <span class="text-surface-400 font-normal">(optional)</span></label>
                  <input v-model="form.action_config.description" placeholder="Additional details about {target_name}..." :class="inputClass" />
                  <div class="flex flex-wrap gap-1 mt-1.5">
                ...
                <div>
                  <label :class="smallLabelClass">Assign to (email)</label>
                  <input v-model="form.action_config.assign_to_email" type="email" placeholder="colleague@company.com" :class="inputClass" />
                  <p class="text-[10px] text-surface-400 mt-1">Leave empty to assign to yourself</p>
                </div>
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    <label :class="smallLabelClass">Due in (days)</label>
              ...
              <template v-if="form.action_type === 'send_chat_message'">
                <div>
                  <label :class="smallLabelClass">Message</label>
                  <textarea v-model="form.action_config.message" placeholder="e.g. {moodboard_name} has been marked as ready by {marked_ready_by}" rows="3" :class="inputClass" />
                  <div class="flex flex-wrap gap-1 mt-1.5">
                    <button v-for="v in templateVars" :key="v.key" @click="insertVarToField('message', v.key)"
                      class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium bg-sky-50 dark:bg-sky-500/10 text-sky-600 dark:text-sky-400 hover:bg-sky-100 dark:hover:bg-sky-500/20 transition-colors cursor-pointer">
                      <span class="material-symbols-rounded" style="font-size:10px">{{ v.icon }}</span>{{ v.key }}
                ...
                <div>
                  <label :class="smallLabelClass">Send to <span class="text-surface-400 font-normal">(optional — leave empty for auto DM)</span></label>
                  ...
                  <div class="rounded-xl border border-surface-200 dark:border-surface-600 bg-white dark:bg-[rgb(var(--color-bg))] overflow-hidden">
                    ...
                    <div v-if="chatConversationsLoading" class="px-4 py-6 text-center">
                      <span class="material-symbols-rounded text-xl text-surface-300 animate-spin">progress_activity</span>
                      <p class="text-xs text-surface-400 mt-1">Loading conversations...</p>
                    </div>
                    ...
                    <div v-else-if="!filteredChatConversations.length" class="px-4 py-6 text-center">
                      ...
                      <p class="text-xs text-surface-400 mt-1">{{ chatSearch ? 'No chats match your search' : 'No conversations found' }}</p>
                    </div>
                    ...
                    <div v-else class="max-h-[220px] overflow-y-auto py-1 px-1">
                      ...
                      >
                        ...
                        <div class="min-w-0 flex-1">
                          <p class="text-sm font-medium">Auto DM</p>
                          <p class="text-[10px] text-surface-400 leading-tight">Send as direct message to the target contact</p>
                        </div>
                      ...
                      >
                        ...
                        <!-- Type badge -->
                        <span v-if="conv.type === 'channel' || conv.is_public" class="text-[9px] uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded bg-surface-100 dark:bg-surface-700 text-surface-500 flex-shrink-0">Channel</span>
                        <span v-else-if="conv.type === 'group'" class="text-[9px] uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded bg-surface-100 dark:bg-surface-700 text-surface-500 flex-shrink-0">Group</span>
              ...
              <template v-if="form.action_type === 'reassign_deals'">
                <div>
                  <label :class="smallLabelClass">New Deal Owner (email)</label>
                  <input v-model="form.action_config.new_owner_email" type="email" placeholder="new-owner@company.com" :class="inputClass" />
                  <p class="text-[10px] text-surface-400 mt-1">All matching deals will be reassigned to this colleague</p>
                </div>
              </template>
            </div>
          </div>
      ...
      <!-- Email Body Editor (full-width, only when action is send_email) -->
      <div v-if="form.action_type === 'send_email'" class="bg-white dark:bg-[rgb(var(--color-surface))] rounded-2xl border border-surface-200 dark:border-[rgb(var(--color-border))] overflow-hidden">
        <div class="px-5 py-4 border-b border-surface-200 dark:border-[rgb(var(--color-border))] flex items-center justify-between">
          ...
          <span class="text-xs text-surface-400">Click a variable to insert it</span>
        </div>
