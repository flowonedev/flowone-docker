...
// Tab state
const activeTab = ref('company') // 'company', 'groups', or 'manual'
...
watch(() => props.show, (isShown) => {
  if (isShown && !props.isEditing) {
    ...
    // Load company users and groups when modal opens
    loadCompanyUsers()
...
async function save() {
  ...
  if (!props.isEditing) {
    ...
    if (activeTab.value === 'company') {
      ...
      // Emit save for each selected user
      for (const userEmail of selectedCompanyUsers.value) {
        emit('save', {
      ...
      >
        ...
        >
                  ...
                  <div class="max-h-48 overflow-y-auto">
                    ...
                    >
                      <div class="flex items-center gap-3 min-w-0">
                        <div class="w-8 h-8 rounded-full bg-primary-100 dark:bg-primary-500/20 flex items-center justify-center text-primary-600 dark:text-primary-400 font-medium text-sm shrink-0">
          ...
          <div class="px-4 sm:px-6 py-3 sm:py-4 border-t border-surface-200 dark:border-surface-700 flex justify-end gap-2 sm:gap-3 shrink-0">
            ...
            <button
              @click="save"
              :disabled="saving || (!isEditing && activeTab === 'manual' && !email.trim()) || (!isEditing && activeTab === 'company' && selectedCompanyUsers.length === 0) || (!isEditing && activeTab === 'groups' && !selectedGroupId)"
