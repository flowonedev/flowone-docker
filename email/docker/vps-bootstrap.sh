...
#   --login-user=<u>       default login mailbox local part (default: robert; env MAIL_LOGIN_USER).
#                          Mirrors native Fleet: every box gets one working IMAP login out of the box.
#   --login-pass=<p>       default login password (default: auto-generated; env MAIL_LOGIN_PASS/ADMIN_PASS).
#                          Saved to <stack-dir>/DEFAULT-LOGIN.txt and printed at the end.
#   --no-login-account     do NOT auto-create the default login mailbox
...
for arg in "$@"; do
    case "$arg" in
        ...
        --login-user=*)        LOGIN_USER="${arg#*=}" ;;
        --login-pass=*)        LOGIN_PASS="${arg#*=}" ;;
        --no-login-account)    CREATE_LOGIN=0 ;;
        --ssl)                 ENABLE_SSL=1 ;;
...
# Default login mailbox — parity with native Fleet resolveMailLogin(): every
# provisioned box gets ONE real IMAP mailbox (robert@<mail-domain> by default) so
# the Email app has a working login immediately. Local part sanitised to a valid
# mailbox part; password from --login-pass / MAIL_LOGIN_PASS / ADMIN_PASS, else
# auto-generated and saved to DEFAULT-LOGIN.txt after boot.
[ -n "$LOGIN_USER" ] || LOGIN_USER="${MAIL_LOGIN_USER:-robert}"
LOGIN_USER="$(printf '%s' "$LOGIN_USER" | tr '[:upper:]' '[:lower:]' | tr -cd 'a-z0-9._-')"
[ -n "$LOGIN_USER" ] || LOGIN_USER="robert"
[ -n "$LOGIN_PASS" ] || LOGIN_PASS="${MAIL_LOGIN_PASS:-${ADMIN_PASS:-}}"
[ -n "$LOGIN_PASS" ] || LOGIN_PASS="$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 20)"
...
docker run --rm -v "${JWT_VOLUME}":/jwt alpine:3 sh -c '
    ...
    chmod 640 /jwt/jwt-private.pem; chmod 644 /jwt/jwt-public.pem
...
if [ "$healthy" = "1" ]; then
    ...
    # ---- 9. Default login mailbox (parity with native Fleet seedMailAccount) ----
    # Create one real IMAP mailbox so the Email app has a working login out of the
    # box, exactly like the native provisioner minted robert@<domain>. Webmail login
    # is a live IMAP auth against the mail pod, so this mail_accounts row is all it
    # needs. Non-fatal: a slow/unseeded mail DB just leaves it to the operator.
    LOGIN_EMAIL=""; CRED_FILE=""
    if [ "$CREATE_LOGIN" = "1" ] && printf '%s' "$MAIL_DOMAIN" | grep -Evq '^[0-9]+(\.[0-9]+){3}$'; then
        LOGIN_EMAIL="${LOGIN_USER}@${MAIL_DOMAIN}"
        log "Creating default login mailbox ${LOGIN_EMAIL} (parity with native 'robert@domain')..."
        if bash "${SCRIPT_DIR}/create-mail-account.sh" --email="${LOGIN_EMAIL}" \
                --password="${LOGIN_PASS}" --quota-mb=2048 --stack-dir="${STACK_DIR}" >/dev/null 2>&1; then
            CRED_FILE="${STACK_DIR}/DEFAULT-LOGIN.txt"
            { echo "FlowOne default login — generated by vps-bootstrap.sh on $(date -u +%FT%TZ)"
              echo "url:      ${SCHEME}://${DOMAIN}/"
              echo "email:    ${LOGIN_EMAIL}"
              echo "password: ${LOGIN_PASS}"
            } > "$CRED_FILE"
            chmod 600 "$CRED_FILE"
            ok "Default mailbox ready; credentials saved to ${CRED_FILE} (chmod 600)."
        else
            warn "Could not create the default mailbox (mail DB seeded yet?). Make one later: ./create-mail-account.sh --email=you@${MAIL_DOMAIN}"
        fi
        echo ""
    elif [ "$CREATE_LOGIN" = "1" ]; then
        warn "MAIL_DOMAIN is a bare IP (${MAIL_DOMAIN}); skipping default mailbox (a real domain is needed to log in)."
        echo ""
    fi

    if [ -n "$CRED_FILE" ]; then
        echo "=============================================================="
        echo " LOG IN TO THE EMAIL APP"
        echo "   URL:      ${SCHEME}://${DOMAIN}/"
        echo "   Email:    ${LOGIN_EMAIL}"
        echo "   Password: ${LOGIN_PASS}"
        echo "   (also saved to ${CRED_FILE})"
        echo "=============================================================="
        echo ""
    fi

    echo "Create more accounts:      ./create-mail-account.sh --email=you@${MAIL_DOMAIN}"
