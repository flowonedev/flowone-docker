...
// Collapse-to-mini-rail is a desktop affordance. On a phone the list should be
// full-width and the collapse handle is dropped (it used to overlap the
// header's compose button).
...
<template>
  <!-- Mobile: full-width, non-collapsible conversation list. -->
  <OriginalChatSidebar
    v-if="isMobile"
    class="w-full"
  ...
  <!-- Desktop expanded sidebar with collapse handle -->
  <div v-else class="relative flex flex-shrink-0">
    <OriginalChatSidebar
      ...
      @select-conversation="(id) => emit('select-conversation', id)"
    />
    <!-- Collapse handle: anchored bottom-right so it never collides with the
         header's compose button. -->
    <button
      @click="toggleCollapse"
      class="absolute bottom-4 right-3 z-10 w-8 h-8 flex items-center justify-center text-surface-400 bg-white dark:bg-surface-800 shadow-md border border-surface-200 dark:border-surface-700 hover:bg-surface-100 dark:hover:bg-surface-700 hover:text-surface-600 dark:hover:text-surface-200 rounded-full transition-colors"
