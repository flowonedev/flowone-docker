...
<?php
/**
 * Background Mail Backup Runner
 *
 * Spawned by the agent (BackupAction::startBackgroundMailBackup) so mail
 * backups run detached from the agent's single-threaded socket loop. The
 * agent returns a status_id immediately and the panel polls
 * /api/backups/status?status_id=... while this process updates the status
 * file via the shared tracking helpers in BackupAction.
 *
 * Usage: php backup-mail-runner.php '<json-encoded-params>'
 */

// Ensure we're running from CLI
if (php_sapi_name() !== 'cli') {
    file_put_contents('/tmp/backup-runner-error.log', date('Y-m-d H:i:s') . " - mail-runner: not running from CLI\n", FILE_APPEND);
    exit(1);
}
...
if (isset($argv[1])) {
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        file_put_contents('/tmp/backup-runner-error.log', date('Y-m-d H:i:s') . " - mail-runner: invalid JSON: " . json_last_error_msg() . "\n", FILE_APPEND);
        exit(1);
    }
}
...
if (!$domain || !$statusId) {
    file_put_contents('/tmp/backup-runner-error.log', date('Y-m-d H:i:s') . " - mail-runner: missing required parameters\n", FILE_APPEND);
    exit(1);
}
...
try {
    ...
    $logger->info("Background mail backup started", [
        'domain' => $domain,
        'status_id' => $statusId,
        'actor' => $actor,
    ]);
    ...
    $result = $backup->execute('backupMail', $backupParams, $actor);

    $logger->info("Background mail backup completed", [
        'domain' => $domain,
    ...
    updateStatusFile($statusId, [
        ...
        'message' => 'Mail backup failed: ' . $e->getMessage(),
    ]);
